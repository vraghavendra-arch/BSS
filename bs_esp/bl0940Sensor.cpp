 ...
 * 
*/
#if MULTIPLE_SERIAL_PORTS_FOR_BL0940

bl0940::bl0940(SoftwareSerial* hwSerial)
{
    this->_serial = hwSerial;
    static_cast<SoftwareSerial*>(_serial)->begin(BL0940_DEFAULT_BAUD_RATE);
...
bl0940::bl0940()
{
    Serial.begin(BL0940_DEFAULT_BAUD_RATE);
#if MULTIPLE_SERIAL_PORTS_FOR_BL0940
    hardwareSerial = true;
#endif
    _init();
...
//for shunt resistance value
void bl0940::setResistance(double resistance){

    _shuntResistance = resistance;
}


double bl0940::getResistance(){

    return _shuntResistance;
}
...
float bl0940::getVoltageRaw(){
    return (((uint32_t)_rawHolder[BL0940_VRMS_HOLD_POS+2]<<16) | 
            ((uint32_t)_rawHolder[BL0940_VRMS_HOLD_POS+1]<<8) |
            ((uint32_t)_rawHolder[BL0940_VRMS_HOLD_POS]));
}
...
float bl0940::getCurrent()
{
    return (((uint32_t)_rawHolder[BL0940_IRMS_HOLD_POS+2]<<16) | 
            ...
            ((uint32_t)_rawHolder[BL0940_IRMS_HOLD_POS])) * (float)BL0940_I_RMS_COEFFICIENT / (float)_shuntResistance;
}

float bl0940::getCurrentRaw(){
    return (((uint32_t)_rawHolder[BL0940_IRMS_HOLD_POS+2]<<16) | 
            ((uint32_t)_rawHolder[BL0940_IRMS_HOLD_POS+1]<<8) | 
            ((uint32_t)_rawHolder[BL0940_IRMS_HOLD_POS]));
}
...
float bl0940::getActivePower()
{
    ...
    return (float)powerValue * (float)BL0940_WATT_COEFFICIENT * (float)BL0940_VOLTAGE_DIVIDER/ (float)_shuntResistance;
}
 ...
 *                                                    3600000 * _shuntResistance
 */
float bl0940::getEnergy(uint32_t cf)
{
    return (float)cf * ((BL0940_ACTIVE_ENERGY_COEFFICIENT * BL0940_WATT_COEFFICIENT * (float) BL0940_VOLTAGE_DIVIDER) / ( 3600000.0 * (float) _shuntResistance ));
}
...
void bl0940::_sendFrame()
{
    ...
    if(hardwareSerial)
    {
        Serial.write(_frame.Payload, _frame.Index);
        return;
    }
    ...
    while( (_frame.Index < _frame.Bytes) && !((millis()-_frame.lastRcv) > BL0940_TIMEOUT))
    {
        ...
        if( bytesAvailable > 0)
        {
            for( int bytesIndex=0; bytesIndex<bytesAvailable; bytesIndex++)
            {
                ...
                if(hardwareSerial)
                {
                    _frame.Payload[_frame.Index++] = (uint8_t)Serial.read();
                }
                else
                {
                ...
                // Serial.println(_frame.Payload[_frame.Index]);
            }
...
void bl0940::reset(){
    uint32_t data = 0x5A5A5A;

    uint8_t txData[6] = {0xA8, BL0940_SOFT_RESET_REG_ADDR, (uint8_t)(data), (uint8_t)(data >> 8), (uint8_t)(data >> 16) };
#if MULTIPLE_SERIAL_PORTS_FOR_BL0940
    if(hardwareSerial){
        Serial.write(txData, 5);
        return;
    }
    _serial->write(txData, 5);
#else
    Serial.write(txData, 5);
#endif
}
